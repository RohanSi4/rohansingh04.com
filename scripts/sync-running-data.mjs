...
const raceFile = path.join(marathonRoot, "data", "race.json");

// The race block is OWNED by the coach (marathon-prep-bot/lib/config.ts) and
// serialized to data/race.json on every publish. This script used to keep its own
// literal copy — and it was the copy that reaches the phone, so when he committed to
// sub-3:30 on 2026-09-05 the Today app and Lock Screen widget kept showing 3:45:00
// and 8:35 /mi for three days while every training pace in the coach had re-keyed.
// There is deliberately NO fallback: a missing file fails the publish, because a
// silently stale goal on his phone is worse than a publish that stops and says why.
const RACE_FIELDS = ["name", "date", "distanceMi", "goalTime", "goalPace", "trainingStart"];
async function loadRace() {
  let raw;
  try {
    raw = await readFile(raceFile, "utf8");
  } catch {
    throw new Error(
      `Race block not found at ${raceFile}. It is generated by the coach — run `
      + `\`npm run publish-dashboard\` in marathon-prep-bot, which writes it from lib/config.ts.`,
    );
  }
  const race = JSON.parse(raw);
  const missing = RACE_FIELDS.filter((field) => race[field] == null);
  if (missing.length > 0) {
    throw new Error(`Race block at ${raceFile} is missing: ${missing.join(", ")}.`);
  }
  return race;
}
